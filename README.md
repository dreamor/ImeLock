...
- macOS 13.0 (Ventura) 或更高版本（需要 SMAppService API）
...
3. 状态栏图标显示锁的状态：
   - lock.fill（实心锁）：输入法已锁定
   - lock.open（打开的锁）：输入法未锁定
...
├── ContentView.swift         # SwiftUI 弹出窗口界面（含 InputSourceRow、ExitButton 等组件）
├── Info.plist               # 应用配置
├── ImeLock.entitlements     # 权限配置
└── Assets.xcassets          # 资源文件（图标等）

ImeLockTests/
└── ImeLockTests.swift       # 单元测试（Swift Testing 框架）
```
...
使用 `os.Logger` 统一日志，subsystem 为 `com.imelock.app`：

| Category | 用途 |
|----------|------|
| `InputMethodManager` | 输入法切换、锁定、恢复相关日志 |
| `AppDelegate` | 状态栏、弹出窗口、用户交互日志 |
| `ContentView` | UI 交互、开机启动设置日志 |
